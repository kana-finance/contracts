...
// ─── Rescue Strategy ─────────────────────────────────────────────────────────
//
// The old deployed vaults (KanaVault + SEIVault) have a strict 0-tolerance
// withdrawal check: if the lending protocol returns even 1 wei less than
// requested, the vault reverts.  Takara's integer exchange rate math produces
// exactly that 1-wei shortfall for any requested amount, making the old vaults
// permanently un-redeemable through the normal path.
//
// Fix: swap the old vault's strategy to this no-op pass-through first.
// setStrategy() pulls funds from Takara into the vault (1-wei loss tolerated
// because setStrategy does not assert the returned amount), then forwards
// the full balance here.  Subsequent redeem() calls work because this
// contract transfers exactly the requested amount with no rounding.
//
// Deployed once per asset, then abandoned after the rescue is complete.
contract RescueStrategy is IStrategy {
    ...
    /// @inheritdoc IStrategy
    function harvest(uint256[] calldata) external pure override returns (uint256) {
        return 0;
    }
...
/// @notice Full redeploy script: rescues funds from v1 vaults, deploys v2, re-deposits.
/// @dev Run dry (no --broadcast) first to verify balances and check for reverts.
contract RedeployAllV2 is Script {
    ...
    // ─── SEI Mainnet Protocol Addresses ─────────────────────────────────────────
    ...
    // ─── Old (v1) Contract Addresses ────────────────────────────────────────────
    address constant OLD_KANA_VAULT = 0x42f6D256d3599b47E632bD20a305D1A1B66baC1d;
    address constant OLD_SEI_VAULT  = 0x3468982893352808d1e248b98A1c5bab0f2833BC;

    // ─── New Contracts (set after deploy) ───────────────────────────────────────
    ...
    function run() external {
        ...
        console.log("=== KANA FULL REDEPLOY V2 ===");
        ...
        // ─── Phase A: Rescue USDC from old vault ────────────────────────────────
        console.log("");
        console.log("--- Phase A: Rescuing USDC from old KanaVault ---");
        KanaVault oldKanaVault = KanaVault(OLD_KANA_VAULT);
        uint256 usdcShares = oldKanaVault.balanceOf(deployer);
        console.log("Old KanaVault shares:", usdcShares);
        uint256 rescuedUsdc = 0;
        if (usdcShares > 0) {
            // The old vault's strict 0-tolerance check makes direct redeem() impossible:
            // Takara's integer exchange rate math returns 1 wei less than requested for
            // any withdrawal amount.  Work around it by swapping the strategy first:
            //   1. Deploy a no-op pass-through RescueStrategy
            //   2. setStrategy() pulls funds from Takara into the vault (without asserting
            //      the exact amount), then forwards them to the rescue strategy
            //   3. redeem() now works — rescue strategy returns exactly what's requested
            RescueStrategy usdcRescue = new RescueStrategy(USDC, address(oldKanaVault));
            oldKanaVault.setStrategy(IStrategy(address(usdcRescue)));
            console.log("Switched old KanaVault to RescueStrategy, funds are now unlocked");

            uint256 usdcBefore = IERC20(USDC).balanceOf(deployer);
            oldKanaVault.redeem(usdcShares, deployer, deployer);
            rescuedUsdc = IERC20(USDC).balanceOf(deployer) - usdcBefore;
            console.log("Rescued USDC (raw):", rescuedUsdc);
        } else {
            console.log("No shares to redeem from old KanaVault");
        }
        ...
        // Deposit all rescued USDC (or minimum seed if nothing was rescued)
        uint256 usdcDeposit = rescuedUsdc > 0 ? rescuedUsdc : 1e6;
        IERC20(USDC).approve(address(newKanaVault), usdcDeposit);
        newKanaVault.deposit(usdcDeposit, deployer);
        console.log("USDC deposited into new vault:", usdcDeposit);
        ...
        if (wseiShares > 0) {
            // Apply the same rescue strategy pattern for WSEI
            RescueStrategy wseiRescue = new RescueStrategy(WSEI, address(oldSEIVault));
        ...
        // Deposit all rescued WSEI (or minimum seed if nothing was rescued)
